...
/** Offline helper: extractive grounded answer from the top case note(s). */
export async function ragAnswer(query: string): Promise<{ text: string; citations: string[] }> {
  const { hits } = await ragContext(query, 3);
  if (!hits.length) return { text: "I couldn't find a relevant case note.", citations: [] };
  const top = hits[0];
