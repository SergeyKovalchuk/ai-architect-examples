...
System_Ext(llm, "Azure OpenAI / OpenAI-compatible gateway", "LLM (tool orchestration + answers) and text embeddings")
...
System_Ext(llm, "Azure OpenAI / OpenAI-compatible gateway", "LLM + embeddings")
...
System_Ext(llm, "Azure / OpenAI-compatible", "LLM + embeddings")
...
Container_Boundary(api, "API + Agent (Node process)") {
  ...
  Component(provider, "provider.ts + dial.ts", "Model wiring", "Builds Azure or OpenAI-compatible chat model + system prompt")
...
sequenceDiagram
    ...
    participant LLM as Azure / OpenAI-compatible
...
flowchart TD
    ...
    TOOLS --> OFF{"OFFLINE?"}
    OFF -->|"0 (live)"| EXT["Open-Meteo · Google News · Azure / gateway embeddings"]
